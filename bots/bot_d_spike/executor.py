"""Executor for Bot D-Spike paper and capped live-probe modes."""
...
from bots.bot_d_spike.strategy import EntryDecision
...
class SpikeExecutor:
        ...
        with self._sessions() as s:
            orders = s.scalar(
                select(func.count()).select_from(Order).where(
                    Order.bot_id == cfg.BOT_D_SPIKE_BOT_ID,
            ...
            positions = s.scalar(
                select(func.count()).select_from(Position).where(
                    Position.bot_id == cfg.BOT_D_SPIKE_BOT_ID,
    ...
    def deployed_usd(self) -> Decimal:
        return Portfolio(self._sessions).get_total_exposure(cfg.BOT_D_SPIKE_BOT_ID)
    ...
    def todays_gross_usd(self, now: datetime | None = None) -> Decimal:
        now = now or datetime.now(UTC)
        start = now.replace(hour=0, minute=0, second=0, microsecond=0)
        with self._sessions() as s:
            total = s.scalar(
    ...
    def _blocker(self, decision: EntryDecision, size_usd: Decimal) -> str | None:
        ...
        if not self._effective_paper() and cfg.PAPER_ONLY:
            return "paper_only_guard"
        if not self._effective_paper() and not cfg.LIVE_APPROVED_AT:
            return "missing_live_approval"
        if cfg.ENTRY_HALT:
            return "entry_halt"
    ...
    def try_enter(self, decision: EntryDecision) -> SpikeEntryResult:
        ...
        effective_paper = self._effective_paper()
        if not resp.order_id:
            return SpikeEntryResult(False, f"skipped:{resp.status or 'NO_ORDER_ID'}")
        if effective_paper and not str(resp.order_id).startswith("paper-"):
            log.error("bot_d_spike.paper_guard_failed response=%s", resp)
            return SpikeEntryResult(False, "paper_order_guard_failed")
        ...
        with self._sessions() as s:
            ...
            s.add(
                Order(
                    ...
                    bot_id=cfg.BOT_D_SPIKE_BOT_ID,
                    condition_id=candidate.market.condition_id,
                    token_id=candidate.yes_token_id,
                    side="BUY",
                    price=limit,
                    size=size_shares,
                    status=resp.status or ("PAPER_OPEN" if effective_paper else "OPEN"),
                    order_type="GTC",
                )
            )
            s.add(
                Event(
                    bot_id=cfg.BOT_D_SPIKE_BOT_ID,
                    event_type="bot_d_spike.entry_placed",
                    severity="info",
                    message=(
                        f"{'paper' if effective_paper else 'live'} BUY placed "
                        f"{candidate.city} {candidate.market.bucket}"
                    ),
                    payload={
                        "condition_id": candidate.market.condition_id,
        ...
        fills = 0
        if effective_paper and self.auto_fill_paper:
            fills = Portfolio(self._sessions).simulate_paper_fills(cfg.BOT_D_SPIKE_BOT_ID)
        return SpikeEntryResult(
            True,
            "placed",
            order_id=resp.order_id,
    ...
    def reconcile_resolutions(self) -> int:
        return Portfolio(self._sessions).reconcile_paper_resolutions(cfg.BOT_D_SPIKE_BOT_ID)

    def reconcile_live_fills(self) -> int:
        if self._effective_paper():
            return 0
        return Portfolio(self._sessions).reconcile_live_fills(
            self.clob,
            cfg.BOT_D_SPIKE_BOT_ID,
